...
public:
    ...
    friend std::istream& operator>>(std::istream& is, Position& position); // For intput

    // operators
    Position& operator+(int diff)
    {
        m_salary += diff;
        return *this;
    }
    ...
    {
        return (m_responsibility == other.m_responsibility && m_salary < other.m_salary);
    }
